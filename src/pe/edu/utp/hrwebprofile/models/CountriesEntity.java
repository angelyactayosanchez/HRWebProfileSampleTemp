...
public class CountriesEntity extends BaseEntity {
    ...
    public Country findById(String id, RegionsEntity regionsEntity) {
        return findByCriteria(
                String.format("WHERE country_id = '%s'", id), regionsEntity).get(0);
    }
    ...
    public Country findByName(String name, RegionsEntity regionsEntity) {
        return findByCriteria(
                String.format("WHERE country_name = '%s'", name), regionsEntity).get(0);
    }
    ...
    public boolean update(String id, String name, Region region) {
        return executeUpdate(String.format(
                "UPDATE %s SET country_name = '%s', region_id = %d WHERE country_id = '%s'", getTableName(), name, id, region));
    }

    public boolean update(Country country) {
        return update(country.getId(), country.getName(), country.getRegion());
    }
    ...
    public boolean erase(Country country) {
        return executeUpdate(String.format("DELETE FROM %s WHERE country_id = '%s'",
                getTableName(), country.getId(), country.getRegion()));
    }
